function Header({ handleThemeChange, theme }) {
  return (
    <section className="flex flex-col">
      <header className="flex flex-row items-center justify-between">
        <h1 className="dark:text-white">devfinder</h1>
        <div
          className="flex flex-row space-x-3 cursor-pointer group"
          onClick={handleThemeChange}
        >
          <h4 className="font-bold text-gray uppercase tracking-[2.5px] dark:text-white group-hover:text-[#222731] dark:group-hover:text-[#90A4D4]">
            {theme}
          </h4>
          <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg">
            {theme === "dark" ? (
              <path
                ...
                fill="#697C9A"
                fillRule="nonzero"
                className="group-hover:fill-[#222731] dark:fill-white dark:group-hover:fill-[#90A4D4]"
              />
            ) : (
              <g
                fill="#FFF"
                fillRule="nonzero"
                className="fill-[#4B6A9B] group-hover:fill-[#222731] dark:fill-white dark:group-hover:fill-[#90A4D4]"
              >
